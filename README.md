...
3. 예제 팔레트 위에 여러 층 적재
4. 적재 완료 감지
5. pickup slide station 아래로 `iw_hub`가 멀리서 접근
6. 팔레트 lift-up
7. 목표 위치로 이송
8. drop slide station에 lift-down 후 AMR 이탈

데모 스크립트는 pickup/drop 위치에 중앙 AMR 통로가 있는 rail/tunnel형 slide station을 생성합니다. `iw_hub`, station, 팔레트 높이는 스크립트의 명명된 상수와 USD bounding box 측정으로 정렬하며, KLT/bin box는 팔레트와 어울리는 따뜻한 tan 계열 색으로 재질을 덮어 시각적으로 구분합니다.
