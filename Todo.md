...
- [x] strict self-test에서 프레임 간 최대 이동량을 실패 조건으로 검증하도록 했다.
  - Python: `--self-test-min-motion-continuity-sample-count`
  - Python: `--self-test-max-amr-frame-displacement`
  - Python: `--self-test-max-active-bin-frame-displacement`
  - Python: `--self-test-max-carried-payload-frame-displacement`
  - PowerShell wrapper와 strict wrapper에도 동일 gate를 연결했다.
- [x] 첫 strict 실행에서 active box가 로봇팔 FK를 따라가던 중 한 프레임에 `0.8360 m` 이동하는 문제가 검출되었다.
- [x] attached active box가 목표 FK pose로 바로 순간이동하지 않도록 `ACTIVE_BIN_ATTACHED_MAX_FRAME_STEP = 0.05`를 적용해 프레임별 이동량을 제한하고 부드럽게 보간했다.
- [x] strict 기준을 더 엄격하게 잡아 active box 최대 프레임 이동량을 `0.08 m` 이하로 검증한다.
- [x] 검증 완료
  - py_compile 통과
  - unittest 80개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_motion_continuity_strict_008_full_e2e_12000.log`
...
- [x] 로봇팔이 실제로 집는 active bin이 spawn 위치에서 pick station으로 순간 이동해 보일 수 있는 문제를 줄이기 위해 scripted conveyor approach를 추가했다.
- [x] active bin은 `CONVEYOR_PICK_WINDOW_Y`에서 `PICK_STATION_BIN_POSITION`까지 `0.60`초 동안 smoothstep 보간으로 이동한 뒤 다음 frame부터 pick 대상으로 허용된다.
- [x] 접근 중인 active bin을 `demo_infeed_active_bin`으로 보존해 review GIF top view에서도 움직이는 pick 대상 박스가 보이도록 했다.
- [x] strict self-test gate를 추가했다.
  - Python: `--self-test-min-active-bin-conveyor-approach-count`
  - Python: `--self-test-min-active-bin-conveyor-observed-travel`
  - Python: `--self-test-max-active-bin-conveyor-final-error`
  - Python: `--self-test-max-active-bin-conveyor-lateral-error`
  - Python: `--self-test-max-active-bin-conveyor-belt-support-gap`
  - PowerShell wrapper와 strict wrapper에도 동일 gate를 연결했다.
- [x] 검증 완료
  - py_compile 통과
  - unittest 79개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_active_bin_conveyor_strict_full_e2e_12000.log`
...
- [x] 사용자가 매 실행 결과를 볼 수 있도록 strict self-test에서 review GIF 저장을 필수 gate로 승격했다.
- [x] `--self-test-require-review-gif`를 추가해 timestamp GIF, `latest_review.gif`, 실제 캡처 frame 수를 검사한다.
- [x] `run_harim_demo.ps1`에 `-SelfTestRequireReviewGif`를 연결하고, `run_harim_strict_self_test.ps1`에서는 항상 `$true`로 켰다.
- [x] self-test 완료 summary에 GIF canvas 크기와 frame 수를 남기도록 했다.
- [x] 검증 완료
  - py_compile 통과
  - unittest 78개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_require_review_gif_strict_full_e2e_12000.log`
...
- [x] feed carton은 실제 로봇팔이 집는 active bin과 물리적으로 섞이지 않도록 collision 없는 visual로 두고, `INFEED_STOP_LINE_Y` 앞에는 들어오지 않게 path를 제한했다.
- [x] 상자 본체와 상단 테이프 visual을 컨베이어 벨트 위에 올리고, guide rail 사이 clearance와 belt support gap을 self-test metric으로 기록한다.
- [x] strict self-test gate를 추가했다.
  - Python: `--self-test-min-infeed-feed-carton-count`
  - Python: `--self-test-min-infeed-feed-carton-observed-travel`
  - Python: `--self-test-min-infeed-feed-carton-stop-clearance`
  - Python: `--self-test-min-infeed-feed-carton-guide-clearance`
  - Python: `--self-test-max-infeed-feed-carton-belt-support-gap`
  - PowerShell wrapper와 strict wrapper에도 동일 gate를 연결했다.
- [x] 검증 완료
  - py_compile 통과
  - unittest 77개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_infeed_feed_carton_strict_full_e2e_12000.log`
...
- [x] `DemoGifRecorder.save()`에서 timestamp GIF 저장 직후 `latest_review.gif`를 갱신하고, 로그에 `latest review GIF updated:`를 출력하도록 했다.
- [x] self-test 완료 summary에 `latest_review_gif_path=`를 추가해 실패/성공 로그만 봐도 최신 GIF 위치를 확인할 수 있게 했다.
- [x] 검증 완료
  - py_compile 통과
  - unittest 74개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_latest_review_gif_strict_full_e2e_12000.log`
...
- [x] 기존 정적 `safety_fence_amr_gate_clearance`에 더해, 실제 AMR pose의 lateral error를 반영한 동적 clearance를 self-test gate로 묶었다.
- [x] strict self-test gate를 추가했다.
  - Python: `--self-test-min-amr-cell-gate-clearance`
  - PowerShell wrapper와 strict wrapper에도 동일 gate를 연결했다.
- [x] 검증 완료
  - unittest 72개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_amr_cell_gate_clearance_strict_full_e2e_12000.log`
...
- [x] 하역 후 팔레트 deck/runner/block/top support 및 load-restraint part가 deck 기준 offset을 유지하는지 `max_dropped_pallet_part_pose_error`로 기록하도록 했다.
- [x] 팔레트 조각이 누락되거나 분리되는 문제를 잡기 위해 `dropped_pallet_part_count`를 self-test metric으로 추가했다.
- [x] strict self-test gate를 추가했다.
  - Python: `--self-test-min-dropped-pallet-part-count`
  - Python: `--self-test-max-dropped-pallet-part-pose-error`
  - PowerShell wrapper와 strict wrapper에도 동일 gate를 연결했다.
- [x] 검증 완료
  - unittest 70개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_dropped_pallet_parts_strict_full_e2e_12000.log`
...
- [x] `max_carried_payload_pose_error`로 운반 중 박스가 팔레트에서 미끄러지거나 좌표가 밀리는 문제를 self-test에서 잡을 수 있게 했다.
- [x] self-test gate를 추가했다.
  - Python: `--self-test-max-carried-payload-pose-error`
  - PowerShell wrapper와 strict wrapper에도 동일 gate를 연결했다.
- [x] 검증 완료
  - unittest 66개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_carried_payload_strict_full_e2e_12000.log`
...
- [x] 같은 시점의 lift fork top과 pallet deck underside 사이의 실제 높이 차이를 `max_pickup_handoff_lift_gap`, `max_pickup_handoff_lift_penetration`으로 기록하도록 했다.
- [x] self-test gate를 추가했다.
  - Python: `--self-test-max-pickup-handoff-xy-error`
  - Python: `--self-test-max-pickup-handoff-lift-gap`
  - Python: `--self-test-max-pickup-handoff-lift-penetration`
  - PowerShell wrapper와 strict wrapper에도 동일 gate를 연결했다.
- [x] 검증 완료
  - unittest 65개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_pickup_handoff_strict_full_e2e_12000.log`
...
- [x] 팔레트를 내려놓은 직후 실제 pallet center가 목표 drop 작업대 중심과 얼마나 맞는지 `drop_handoff_xy_error`로 기록하도록 했다.
- [x] 팔레트 deck underside와 drop slide support top 사이의 실제 높이 차이를 `drop_handoff_support_gap`, `drop_handoff_support_penetration`으로 기록하도록 했다.
- [x] self-test gate를 추가했다.
  - Python: `--self-test-max-drop-handoff-xy-error`
  - Python: `--self-test-max-drop-handoff-support-gap`
  - Python: `--self-test-max-drop-handoff-support-penetration`
  - PowerShell wrapper와 strict wrapper에도 동일 gate를 연결했다.
...
- [x] 검증 완료
  - unittest 64개 통과
  - 12000-frame strict full end-to-end self-test 통과
  - 로그 파일: `isaacsim_logs/harim_drop_handoff_strict_full_e2e_12000.log`
  - GIF: `isaacsim_outputs/harim_amr_review_20260530_014950_9596.gif`
  - 완료 로그 핵심값: `placed_bins=8`, `transfer_cycles=1`, `drop_handoff_xy_error=0.0000`, `drop_handoff_support_gap=0.0050`, `drop_handoff_support_penetration=0.0000`
