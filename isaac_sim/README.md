...
- `compute_review_gif_layout()`으로 map 영역과 상태 패널 영역을 계산해, 우측 패널 폭을 280px 이상으로 보장합니다.
- GIF export unittest에서 실제 저장된 GIF 크기가 `GIF_CANVAS_SIZE`와 일치하는지 확인합니다.
- 검증 로그: `isaacsim_logs/harim_review_gif_layout_strict_full_e2e_12000.log`
...
- guide rail은 lowered fork부터 raised fork travel 범위까지 덮도록 높이를 계산하고, AMR pose와 함께 동기화합니다.
- strict self-test에 lift guide count, bottom gap, travel cover, pose error gate를 추가했습니다.
- 검증 로그: `isaacsim_logs/harim_amr_lift_guide_strict_full_e2e_12000.log`
...
- 정상 캡처 frame이 하나도 없는 상태에서 종료되어도 fallback frame을 만들어 GIF 파일을 저장합니다.
- GIF 캡처 중 예외가 나면 원인을 기록하고, 기존 frame 또는 fallback frame으로 저장을 계속 시도합니다.
- 완료/실패 로그에는 `review_gif_path=`가 출력됩니다.
